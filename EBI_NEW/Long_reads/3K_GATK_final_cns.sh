...
#or i in *.fq.gz
#do
#seqtk seq -VQ64 ${i} | gzip -c > ${TEMP}/p33_${i} &
#done

#ait && echo "seqtk is done"
...
do
	INPUT_ID="`basename $i|sed "s/_1.fastq.gz//g"`"
	FASTQ1="${INPUT_ID}_1.fastq.gz"
	FASTQ2="${INPUT_ID}_2.fastq.gz"
	bwa mem -M -t ${THREADS} -R "@RG\tID:${SAMPLE_NAME}\tSM:${SAMPLE_NAME}\tLB:${SAMPLE_NAME}\tPL:ILLUMINA\tPU:run" ${REF} $FASTQ1 $FASTQ2 | \
		samtools view -@ ${THREADS} -bq 20 | \
			sambamba sort -t ${THREADS} -m ${MEMORY} -o ${TEMP}/${SAMPLE_NAME}_merge.bam  /dev/stdin	
done

#ait && echo "bwa mem is done"

#ambamba merge --nthreads=${THREADS} ${TEMP}/${SAMPLE_NAME}_merge.bam `ls ${TEMP}/*_sorted_q1.bam`

#echo "sambamba merge is done"

# remove the memory limitation!!!
